...
import {Button, StyleSheet, Text, View, TextInput, ScrollView, Switch} from "react-native";
...
const ApplyNLP = () => {
    ...
    const [isOrderMenu, setIsOrderMenu] = useState(false); //토글 상태태
        ...
        <Container>
            ...
            <TopContainer style={{borderWidth: 1, borderColor: "#00574b"}}>
                ...
                {(orderMenus.length && isOrderMenu) ?
                    orderMenus.map(menu => {
                        return <MenuContainer key={menu.id}>
                            <MenuName>
                                <Text>{menu.name}{'\n\n'}</Text>
                                <MenuPrice>
